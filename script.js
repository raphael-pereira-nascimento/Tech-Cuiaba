...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Close modals
  const closeButtons = ['closeContaModal', 'closeLoginModal', 'closeCadastroModal'];
  closeButtons.forEach(id => {
    const btn = document.getElementById(id);
    if (btn) {
      btn.addEventListener('click', () => closeModal(id.replace('close', '').replace('Modal', '') + 'Modal'));
    }
  });
  ...
  if (cadastroForm) {
    cadastroForm.addEventListener('submit', function(e) {
      ...
      if (email && email.value.includes('@') && password && password.value.length >= 6) {
        usuarioAtual = {
          nome: nome.value,
          email: email.value
        };
